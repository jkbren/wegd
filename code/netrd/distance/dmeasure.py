...
class DMeasure(BaseDistance):
    ...
    def dist(self, G1, G2, w1=0.45, w2=0.45, w3=0.10, niter=50):
        ...
        .. math::
            NND(G) = \frac{\mathcal{J}(\mathbf{P}_1,\ldots,
                                       \mathbf{P}_N)}{\log(d+1)},
        ...
        if not np.isclose(sum([w1, w2, w3]), 1):
...
def node_distance(G):
    ...
    ss = shortest_path_matrix(G)
    for i, s in enumerate(ss):
        countdict = dict(Counter(s))
        if 0.0 in countdict.keys():
            del countdict[0.0]
        paths_of_length_d = np.array(list(countdict.keys())).astype(int) - 1
        number_of_nodes_d = np.array(list(countdict.values())).astype(int)
        a[i, paths_of_length_d] = number_of_nodes_d
...
def network_node_dispersion(G):
    ...
    norm = np.log(max([2, len(np.nonzero(pdfm[:(N-1)])[0]) + 1]))
    ndf = nd.flatten()

    # calculate the entropy, with the convention that 0/0 = 0
    entr = -1 * sum(ndf * np.log(ndf, out=np.zeros_like(ndf),
                                 where=(ndf != 0)))
